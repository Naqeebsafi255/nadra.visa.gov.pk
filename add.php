...
// د اتوماتیک ریفرنس نمبر جوړولو فنکشن
function generateReferenceNumber($conn) {
    do {
        $ref = "8800" . mt_rand(1000000, 9999999);
        
        $stmt = mysqli_prepare($conn, "SELECT id FROM visa_records1_new WHERE visa_reference_number = ?");
...
// د اتوماتیک نمبر تولیدول (که فورم تازه شي یا سیمیټ شي)
$auto_ref = generateReferenceNumber($conn);

if(isset($_POST['submit'])){

    // که اډمین په لاسي ډول بل څه ولیکل، همغه اخلو، که نه نو همدغه اتوماتیک نمبر
    ...
    if(empty($ref) || empty($name) || empty($dob) || empty($passport) || empty($country) || empty($nation) || empty($cat) || empty($subcat) || empty($type) || empty($status) || empty($appdate) || empty($stay)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO visa_records1_new (
        ...
        mysqli_stmt_bind_param($stmt, "ssssssssssssssssss",
            ...
            $grant, $stay, $start, $end, $duration, $photo_name, $status, $appdate, $sex
        );

        if(mysqli_stmt_execute($stmt)){
            $message = "<div class='bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4'>✔ New Visa Record Added Successfully!</div>";
            $auto_ref = generateReferenceNumber($conn);
        } else {
...
<head>
<meta charset="UTF-8">
<title>Add New Visa | Nader System</title>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        ...
        <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-primary">
            <h2 class="text-2xl font-bold text-primary flex items-center gap-2">
                <i class="fa fa-user-plus"></i> Add New Visa Record
            </h2>
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1">Reference Number (Auto / Editable) <span class="text-red-500">*</span></label>
                        <input type="text" name="visa_reference_number" value="<?= htmlspecialchars($auto_ref) ?>" required
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary font-bold text-lg text-primary" placeholder="Enter or keep auto reference">
                    </div>
                    <div>
                        <label class="block mb-1">Full Name <span class="text-red-500">*</span></label>
                        <input type="text" name="full_name" placeholder="Full Name as in Passport" required
            ...
            <div>
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="passport_country" placeholder="e.g. Afghanistan" required
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                    </div>
                    <div>
                        <label class="block mb-1">Nationality <span class="text-red-500">*</span></label>
                        <input type="text" name="nationality" placeholder="e.g. Afghan" required
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                    </div>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-4 mt-1">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="sex" value="M" class="hidden peer" checked>
                            ...
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="sex" value="F" class="hidden peer">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="visa_category" required class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                            ...
                            <option value="Tourist/Visa">Tourist/Visit</option>
                            <option value="Business">Business</option>
                            <option value="Work">Work</option>
                            <option value="Medical">Medical</option>
                            <option value="Student">Study</option>
                            <option value="Family Visa">Family Visit</option>
                            <option value="Diplomatic Visa">Diplomatic / Official</option>
                            <option value="Transit Visa">Transit/Visa</option>
                            <option value="Refugee">Refugee / Humanitarian</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="visa_sub_category" required class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                            ...
                            <option value="Individual (less Than 3 Months)">Individual (less Than 3 Months)</option>
                            <option value="Individual">Individual</option>
                            <option value="Transit/Transport(entry)">Transit/Transport(entry)</option>
                            <option value="General">General</option>
                            <option value="Family Visit">Family Visit</option>
                            <option value="Short-Term">Short-Term</option>
                            <option value="Group Entry">Group Entry - Single Visit</option>
                            <option value="Emergency Entry">Emergency Entry - Short Stay</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="visa_grant_date"
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" name="visa_start_date"
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                    </div>
                    <div>
                        <label class="block mb-1">End Date</label>
                        <input type="date" name="visa_end_date"
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                    </div>
                    <div>
                        <label class="block mb-1">Duration</label>
                        <input type="text" name="visa_duration" placeholder="e.g. 30 Days / 1 Year"
                               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg outline-none focus:border-primary">
                    </div>
                </div>
            </div>
            ...
            <button type="submit" name="submit"
                ...
                <i class="fa fa-plus-circle"></i> Save New Visa Record
            </button>
